...
$Id: LoggingListener.cfc 1049 2008-09-14 21:44:09Z peterfarrell $
...
<cfcomponent
	...
	<cffunction name="getLoggers" access="public" returntype="struct" output="false"
		...
		<cfif IsObject(loggingProperty) AND StructCount(loggingProperty.getLoggers())>
			<cfset data['base'] = loggingProperty.getLoggers() />
		</cfif>
		...
		<cfloop collection="#modules#" item="key">
			...
			<cfif IsObject(loggingProperty) AND StructCount(loggingProperty.getLoggers())>
				<cfset data[modules[key].getModuleName()] = loggingProperty.getLoggers() />
			</cfif>
		</cfloop>
	...
	<cffunction name="enableDisableAll" access="public" returntype="void" output="false"
		...
		<cfif arguments.event.getArg("mode") EQ "enable">
			...
			<cfset arguments.event.setArg("message", "Enabled all loggers.") />
		<cfelse>
			<cfset getAppManager().getLogFactory().disableLogging() />
			<cfset arguments.event.setArg("message", "Disabled all loggers.") />
		</cfif>
	</cffunction>
	...
	<cffunction name="enableDisableLogger" access="public" returntype="void" output="false"
		...
		<cfif mode EQ "enable">
			...
			<cfset arguments.event.setArg("message", "Enabled '#loggerName#'.") />
		<cfelse>
			<cfset logger.setLoggingEnabled(false) />
			<cfset arguments.event.setArg("message", "Disabled '#loggerName#'.") />
		</cfif>
	</cffunction>
	...
	<cffunction name="changeLoggingLevel" access="public" returntype="void" output="false"
		...
		<cfset arguments.event.setArg("message", "Changed level in '#loggerName#' to '#level#'.") />
	</cffunction>
