<cfcomponent displayname="Application.cfc" 
	...
	<cfset settingsFile = ExpandPath("./config/settings.xml") />
	<cffile action="read" file="#settingsFile#" variable="appSettingsWDDX">
	<cfwddx action="wddx2cfml" input="#appSettingsWDDX#" output="appSettings" />
	
	<!--- cf settings --->
	<!--- <cfset this.name = "chug" /> --->
	<cfif StructKeyExists(appSettings.sites.hosts,cgi.http_host)>
		<cfset this.name=appSettings.sites.hosts["#cgi.http_host#"] />
	<cfelse>
		<cfset this.name=appSettings.sites.hosts.default />
	</cfif>
	...
	<!--- mach-ii settings --->
	<cfset configpathlocation=appSettings.sites.applications["#this.name#"] />
	<cfset MACHII_CONFIG_PATH = ExpandPath("#configpathlocation#") />
